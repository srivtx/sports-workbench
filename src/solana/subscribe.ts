...
function getConnection(config: TxLineConfig, overrideRpc?: string): Connection {
  return new Connection(
    overrideRpc ?? config.rpcUrl ?? (config.devnet
...
export async function subscribeOnChain(
  ...
  const connection = getConnection(config, opts.rpcUrl);
...
export async function activateApiToken(
  ...
  const res = await axios.post<{ token?: string; expiresAt?: number }>(
